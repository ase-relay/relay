...
export function LocationSuggestionItem({ suggestion, onSelect, isCurrentLocation = false }: LocationSuggestionItemProps) {
  return (
    <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => onSelect(suggestion)} className="flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-primary-50">
      ...
      <span className="min-w-0"><span className="block truncate text-sm font-medium text-neutral-900">{suggestion.name}</span>{suggestion.district && <span className="block truncate text-xs text-neutral-500">{suggestion.district}</span>}</span>
