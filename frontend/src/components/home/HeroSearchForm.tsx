  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 w-full max-w-md">
      <div className="relative">
        {/* Garis putus-putus penghubung - responsive positioning */}
        <div className="absolute left-3 top-5 bottom-14 sm:top-6 sm:bottom-19 w-px border-l-2 border-dashed border-neutral-300" />

        {/* Input lokasi awal */}
        <div className="relative mb-3 sm:mb-4">
          <div className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 border-neutral-400 bg-white" />
          <input
            type="text"
            ...
            className="w-full pl-9 pr-4 py-2.5 sm:py-3.5 border border-neutral-200 rounded-lg text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-accent-blue focus:border-transparent"
          />
        ...
        {/* Tombol tukar lokasi - responsive positioning */}
        <button
          type="button"
          onClick={() => { handleSwap(); setError(''); }}
          aria-label="Tukar lokasi"
          className="absolute right-1 top-1/2 -translate-y-1/2 p-1.5 rounded-full hover:bg-neutral-100 transition-colors sm:right-0"
        >
        ...
        <div className="relative mb-4 sm:mb-5">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-neutral-500" fill="currentColor" viewBox="0 0 20 20">
      ...
      {error && <p role="alert" className="mt-3 sm:mt-4 text-xs sm:text-sm font-medium text-red-600">{error}</p>}

      <button
        onClick={handleSearch}
        disabled={isSearchDisabled}
        className="w-full flex items-center justify-center gap-2 min-h-[44px] bg-accent-blue text-white py-3 sm:py-3.5 rounded-lg font-semibold hover:opacity-90 transition-opacity disabled:bg-neutral-300 disabled:cursor-not-allowed"
      >
